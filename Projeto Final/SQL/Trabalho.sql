...
CREATE TABLE  Hospital
	(Codigo		INTEGER UNSIGNED PRIMARY KEY AUTO_INCREMENT,
     ...
     Regiao_Codigo INTEGER UNSIGNED references Regiao(Codigo)
     );
...
CREATE TABLE  Funcionarios
	(CPF		INTEGER UNSIGNED PRIMARY KEY,
     ...
     Hospital_Codigo INTEGER UNSIGNED references Hospital(Codigo)
     );
CREATE TABLE  Telefone_Funcionario
	(Telefone		INTEGER UNSIGNED PRIMARY KEY,
     Funcionarios_CPF INTEGER UNSIGNED references Funcionarios(CPF)
     );
...
CREATE TABLE  Hospital_Equipamentos
	(Hospital_Codigo INTEGER UNSIGNED references Hospital(Codigo),
     Equipamentos_Codigo INTEGER UNSIGNED references Equipamentos(Codigo)
     );
...
CREATE TABLE  Hospital_Pacientes
	(Hospital_Codigo INTEGER UNSIGNED references Hospital(Codigo),
     Pacientes_CPF INTEGER UNSIGNED references Pacientes(CPF)
     );
CREATE TABLE  Testes
	(Codigo		INTEGER UNSIGNED PRIMARY KEY AUTO_INCREMENT,
...
CREATE TABLE  Pacientes_Enfermidades
	(Pacientes_CPF INTEGER UNSIGNED references Pacientes(CPF),
     Enfermidade_Nome varchar(65) references Enfermidades(Nome)
     );
...
CREATE TABLE  Pacientes_Medicamentos
	(Pacientes_CPF INTEGER UNSIGNED references Pacientes(CPF),
     Medicamentos_Codigo INTEGER UNSIGNED references Medicamentos(Codigo)
     );
